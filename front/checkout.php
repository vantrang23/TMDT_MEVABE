...
?>
<?php
if (isset($_GET['product_id']) && isset($_GET['tongtien']) && isset($_GET['soluong'])) {
    // Lấy dữ liệu từ URL
    $selectedProducts = $_GET['product_id'];
    $selectedtongtien = $_GET['tongtien'];
    $selectedsl = $_GET['soluong'];

    $_SESSION['selectedsl'] = $selectedsl;
    $_SESSION['selectedProducts'] = $selectedProducts;
    $_SESSION['selectedtongtien'] = $selectedtongtien;
}
?>
<?php
...
if (isset($_POST['user_order'])) {
    ...
    $selectedthanhtien = array();
    foreach ($_POST['tt'] as $product_id => $total_price) {
        $selectedthanhtien[$product_id] = array(
            'product_id' => $product_id,
            'total_price' => $total_price
        );
    }
    $selectedsl = array();
    foreach ($_POST['sl'] as $product_id => $sl) {
        $selectedsl[$product_id] = array(
            'product_id' => $product_id,
            'soluong' => $sl
        );
    }
    ...
    if ($result_them_order) {
        ...
        // Chuỗi SQL chứa tất cả các chi tiết đơn hàng
        $sql_themctgh = "INSERT INTO order_detail (id_order, product_id, soluong, thanhtien) VALUES ";

        foreach ($selectedProducts as $product_id) {
            $soluong = $selectedsl[$product_id]['soluong'];
            $thanhtien = $selectedthanhtien[$product_id]['total_price'];
        ...
        if ($re_themchgh) {
            ...
            header("location: thongtinthanhtoan.php?iduser=$iduser&id_order=$last_inserted_id");
        } else {
                    ...
                    <div class="row">
                        <div class="col-lg-8 col-md-6">
                            <div class="checkout__form-wrapper">
                                ...
                                <div class="checkout__input">
                                    <p>Phí vận chuyển</p>
                                    <input type="hidden" name="phivanchuyen" id="" value="<?php echo isset($shippingFee) ? $shippingFee : '0'; ?>">
                            ...
                            <div class="checkout__order" style="width:400px;">
                                ...
                                <ul>
                                    ...
                                    ?>
                                        ...
                                        <li><?php echo truncateString($row_gh['name'], 30) ?> <span><?php echo number_format($selectedtongtien[$product_id]['total_price'], 0, '', '.') ?></span></li>
                                    <?php } ?>
                                </ul>
                                <div class="checkout__order__total">Tổng tiền hàng <span><?php echo number_format($tong, 0, '', '.'); ?></span></div>
                                <div class="checkout__order__total">Phương thức vận chuyển<span>Nhanh</span></div>
                                <div class="checkout__order__total" id="paymentMethodLabel">Phương thức thanh toán <span></span></div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#paymentMethod').change(function() {
                ...
                var selectedPaymentMethod = $('#paymentMethod option:selected').text();

                // Update the label in the checkout summary
                $('#paymentMethodLabel span').text(selectedPaymentMethod);
            });
        });
    </script>
